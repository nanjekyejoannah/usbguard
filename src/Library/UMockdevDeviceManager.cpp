...
namespace usbguard
{
  ...
  void UMockdevDeviceManager::umockdevRemove(const std::shared_ptr<UMockdevDeviceDefinition>& definition)
  {
    ...
    umockdevRemove(SysFSDevice::getSysfsRoot() + definition->getSysfsPath());
  }
  ...
  void UMockdevDeviceManager::umockdevRemove(const std::string& sysfs_path)
  {
    ...
    umockdev_testbed_remove_device(_testbed.get(), sysfs_path.c_str());
  }
  ...
  void UMockdevDeviceManager::umockdevRemoveByFile(const std::string& definitions_path)
  {
    (void)definitions_path;
    //auto it =
  }

  void UMockdevDeviceManager::umockdevRemoveBySysfsPath(const std::string& sysfs_path)
  {
    (void)sysfs_path;
  }
  ...
  void UMockdevDeviceManager::umockdevDeauthorizeBySysfsPath(const std::string& sysfs_path)
  {
    ...
    for (const auto& child_device_path : umockdevGetChildrenBySysfsPath(sysfs_path)) {
      ...
      umockdevRemove(SysFSDevice::getSysfsRoot() + child_device_path);
    }
  }
  ...
  void UMockdevDeviceManager::umockdevProcessInotify()
  {
    ...
    std::string device_filepath(event->name, event->len);
    USBGUARD_LOG(Debug) << "inotify pathname = " << device_filepath;

    if (event->mask & IN_CREATE) {
      umockdevLoadFromFile(_umockdev_deviceroot + "/" + device_filepath);
    }
    else if (event->mask & IN_DELETE) {
      umockdevRemoveByFile(_umockdev_deviceroot + "/" + device_filepath);
    }
  }
